...
\subsection{The FP8 KV-Cache Decoding Anomaly: Software vs. Hardware Limits}
When transitioning from the embedding extraction benchmark to the pure generative decoding benchmark (Table \ref{tab:generation}), a remarkable anomaly emerges. While quantizing the Key-Value (KV) cache to FP8 successfully preserves biological representational fidelity (NLL: 4.42 $\to$ 4.42) and halves the theoretical memory footprint of the context window, it simultaneously triggers a devastating \textbf{35.6\% regression in decoding throughput} (3214.82 ток/s $\to$ 2067.20 ток/s) relative to native BF16 execution.

This regression is not a defect of the NVIDIA H100 hardware, but rather a profoundly illuminating artifact of the modern LLM software compilation stack—specifically, the dependency chains within the vLLM serving engine on High-Performance Computing (HPC) clusters.

\subsubsection{The Compilation Fallback Penalty}
Achieving theoretical FP8 hardware acceleration during the attention decoding phase requires highly optimized, hand-written C++ CUDA kernels capable of executing fused dequantization and matrix multiplication natively. Leading libraries such as \texttt{FlashInfer} provide this capability. However, deploying these bleeding-edge CUDA extensions requires strict host-compiler synchronization (e.g., matching GCC, NVCC, and Ninja binary toolchains). 

In complex HPC environments (such as the Conda environment utilized in our cluster), these dependencies frequently fracture, resulting in Just-In-Time (JIT) compilation failures. When vLLM detects the absence of a compiled \texttt{FlashInfer} binary, it gracefully but fatally falls back to less optimized attention backends, such as Python-wrapped \texttt{Triton} kernels or standard \texttt{XFormers}. 

Conversely, native BF16 execution on the H100 is universally supported by the pre-compiled, ubiquitous \texttt{FlashAttention-3} library, which achieves near-peak theoretical GPU utilization. Thus, our benchmark essentially races an algorithmically perfect BF16 implementation against a computationally crippled FP8 fallback.

\subsubsection{The On-the-Fly Dequantization Bottleneck}
The performance collapse is further exacerbated by how these fallback kernels handle sub-byte precision. Because the underlying fallback kernels are primarily designed for 16-bit floating-point math, they cannot perform the core Attention matrix operations directly on the 8-bit KV cache. 

During every single autoregressive decoding step, the GPU must:
\begin{enumerate}
    \item Fetch thousands of FP8 KV tensors from High Bandwidth Memory (HBM).
    \item Materialize and cast (dequantize) these tensors into BF16 formats within the SM registers.
    \item Execute the BF16 matrix multiplication inside the Tensor Cores.
    \item Cast the resultant new token state back into FP8 and write it back to HBM.
\end{enumerate}
This continuous, on-the-fly casting overhead dominates the micro-latency of the decoding loop, entirely negating the bandwidth savings gained by fetching smaller 8-bit payloads. 

Furthermore, the observation that VRAM consumption identically peaks at $\sim$76 GB for both precision formats highlights a critical abstraction layer within the vLLM memory manager. The system preemptively reserves 90\% of the global HBM block at initialization (via \texttt{gpu\_memory\_utilization}). Therefore, the FP8 KV cache does not release physical memory back to the OS; rather, it functionally doubles the \textit{internal block capacity} of the pre-allocated pool, allowing the engine to sustain double the concurrent batch size before encountering an Out-Of-Memory (OOM) fault.

Ultimately, these findings prove that while FP8 is a devastatingly effective memory-compression technique for massive genomic contexts, unlocking its theoretical speed advantages requires a pristine, fully synchronized compilation environment capable of deploying custom fused C++ kernels. Without it, researchers must balance the trade-off of doubled capacity against a steep penalty in generation speed.
