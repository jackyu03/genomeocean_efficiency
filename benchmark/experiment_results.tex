...
\begin{abstract}
As genomic foundation models scale towards billions of parameters, the computational cost of inference becomes a critical bottleneck for high-throughput biological discovery. This study systematically evaluates the performance trade-offs of the GenomeOcean architecture on next-generation NVIDIA H100 hardware. By leveraging native hardware support for FP8 precision alongside standard BFloat16, we investigate the profound friction between theoretical hardware acceleration and practical software framework limitations. We document the architectural roadblocks encountered when attempting to apply eight-bit quantization to massive contextual workloads, detailing a necessary technical pivot from native PyTorch environments to specialized high-throughput inference engines. Our results demonstrate that while FP8 quantization offers substantial memory compression, its impact on representational fidelity and raw generation speed varies significantly depending on the interplay between model scale, algorithmic compromises, and suboptimal compiler fallbacks. Ultimately, this work quantifies the impact of quantization on computational efficiency, biological syntax generation, and downstream species binning utility, revealing that achieving theoretical performance ceilings requires navigating severe software dependencies.
\end{abstract}

\section{Introduction}
Genomic Language Models have demonstrated remarkable capabilities in learning the syntax and semantics of biological sequences. However, deploying these models at scale poses significant challenges due to their unprecedented memory footprint and computational latency. The recent advent of hardware-accelerated low-precision inference, notably FP8 support on NVIDIA H100 GPUs, presents a highly anticipated opportunity to mitigate these costs. This work provides a rigorous benchmark of the GenomeOcean family of models, assessing whether reduced-precision inference can maintain the fidelity required for sensitive biological tasks such as metagenomic binning and sequence generation. Beyond standard capability evaluations, this paper meticulously documents the compounding software architecture limitations encountered during deployment. By chronicling the transition from native PyTorch tensor operations to the specialized vLLM serving engine, we expose the underlying technological compromises required to execute extremely long-context biology workloads on modern accelerator silicon.
...
\subsection{The Benchmarking Schism and Operational Protocols}
We established two rigorous benchmarks to evaluate efficiency and quality independently. The bifurcation of our evaluation methodology was strictly necessitated by severe limitations within modern deep learning software frameworks. Initial attempts to benchmark the models utilized the native PyTorch FP8 configuration, specifically leveraging the FbgemmFp8Config module. While this native implementation allowed for the crucial extraction of last-layer hidden state embeddings necessary for downstream clustering tasks, it proved to be computationally inefficient when scaling to the massive context windows required by the GenomeOcean architecture. These native operations consistently failed to adequately saturate the Tensor Cores on the H100 hardware, resulting in suboptimal latency. To bypass this native limitation and achieve genuine scaled throughput, we transitioned our generative benchmarking pipeline to vLLM, a specialized high-throughput inference engine designed specifically for massive parallel batching. However, this strategic pivot introduced a secondary architectural constraint. The vLLM engine is structurally optimized exclusively for autoregressive causal generation and fundamentally lacks the computational pathway to surface internal hidden state embeddings back to the user space. Consequently, our benchmarking protocol was forced to fracture into two completely isolated operational paradigms, designated as Protocol A and Protocol B.

\subsubsection{Protocol A: Native Embedding Utility and Extraction Efficiency}
This initial protocol utilizes the constrained native PyTorch environment to measure both the raw computational performance characteristics of the baseline framework and the semantic quality inherent to the internal representations generated by the model. For these experiments, we curated a specific evaluation dataset consisting of twenty randomly selected genomes sampled from the GTDB Bacteria database. To guarantee statistical robustness across all trials, the evaluation pipeline processed approximately one million tokens per genome. To seamlessly accommodate the highly disparate context windows of the two architectural scales while strictly maintaining comparable data coverage volume, we adopted distinct, model-specific fragmentation strategies. For the GenomeOcean-100M model, expansive genomic fragments were systematically subdivided into non-overlapping chunks of 5000 base pairs. The model subsequently processed these segments as 1024-token sequences. Operating with a sample size of one thousand individual fragments, this specific approach yielded an effective data utilization rate of approximately 96 percent. Conversely, the GenomeOcean-4B model was configured to process completely unmodified fragments by utilizing its full 10240-token context window. Operating with one hundred full-length fragments, this approach achieved nearly comprehensive data utilization. To accurately quantify the semantic utility derived from these processes under the PyTorch framework, we extracted the corresponding last-layer hidden embeddings from the models. We then applied the DBSCAN clustering algorithm to these embeddings, utilizing an epsilon value of 0.5 and a minimum sample threshold of five. The overall downstream performance of this pipeline was rigorously evaluated using the Adjusted Rand Index, determining the exact level of mathematical agreement between the unsupervised model-induced clusters and the actual biological ground-truth taxonomic labels.

\subsubsection{Protocol B: High-Throughput Causal Generation via vLLM}
This secondary protocol leverages the specialized vLLM engine to systematically evaluate the capacity of the model to generate biologically valid sequences, serving as a direct quantitative proxy for its generalized understanding of underlying genomic grammar under extreme hardware loads. For this generation phase, a substantially larger evaluation set comprised of one hundred full genomes was utilized to assure absolute statistical significance across all derived metrics. To evaluate the precise scoring fidelity of the architectures within the inference engine, we computed both standard Perplexity and Negative Log-Likelihood utilizing a sliding window context approach. Furthermore, we conducted automated raw generation throughput stress tests, measuring continuous tokens per second, to evaluate the fundamental hardware efficiency of the autoregressive decoding phase while operating under a maximum parallel load utilizing a batch size of 128. Concurrently, the total electrical power draw in Watts was actively measured throughout the entire vLLM generation cycle. This metric was recorded utilizing continuous polling through the NVIDIA Management Library via a custom energy measurement apparatus, allowing us to accurately establish the exact computational token-per-watt efficiency profile of the hardware outside of the native PyTorch constraints.

\section{Results}

\subsection{Computational Efficiency of Native Embeddings}
Table \ref{tab:efficiency} presents the calculated throughput, listed as continuous tokens per second, arrayed alongside the total electrical power consumption measured in continuous Watts. These metrics isolate the performance characteristics specifically recorded during Protocol A, thereby illuminating the stark computational realities of utilizing the native PyTorch FBGEMM implementation when executing tasks that necessitate the full retrieval of hidden state embeddings across expansive genomic context arrays.
...
\subsection{Downstream Utility of Extracted Representations}
Table \ref{tab:binning} illustrates the direct biological efficacy of the embeddings generated during the constrained native extraction process, specifically measuring their mathematical validity when applied to taxonomic classification tasks.
